...
class Store {
  ...
  Stream<QuerySnapshot> DeleteOrders() {
    _firestore.collection(kOrders).getDocuments().then((snapshot) {
  ...
  deleteAll(){
    _firestore.collection(kProductsCollection).getDocuments().then((snapshot) {
      for (DocumentSnapshot ds in snapshot.documents) {
        ds.reference.delete();
      }
    }
